...
async function ingestTelemetry(droneId, payload = {}) {
  ...
  try {
    ...
    const hubToRestaurant =
      hubCoordinate && branchCoordinate ? metersBetween(hubCoordinate, branchCoordinate) : null;
    const restaurantToCustomer =
      branchCoordinate && customerCoordinate ? metersBetween(branchCoordinate, customerCoordinate) : null;
    ...
    if (deliveryRow) {
      ...
      if (!shouldMarkDelivered && totalCourseDistance > 0) {
        if (deliveryStage === 'to_restaurant') {
          const baseRatio = safeDivide(hubToRestaurant || 0, totalCourseDistance);
          const travelled =
      ...
      if (deliveryStage === 'to_restaurant') {
        const remaining = (distanceToRestaurant ?? 0) + (restaurantToCustomer || 0);
        etaSeconds = estimateEtaSeconds(remaining);
      } else if (deliveryStage === 'arriving') {
        etaSeconds = estimateEtaSeconds(restaurantToCustomer || 0);
      } else if (deliveryStage === 'to_customer') {
        ...
        etaSeconds = estimateEtaSeconds(distanceToHub || 0);
