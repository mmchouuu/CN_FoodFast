...
flowchart LR
  subgraph Clients
    ...
    Resto[Restaurant UI]
    AdminUI[Admin UI]
    AuthClient[Auth SDK and Token storage]
  end
  ...
  subgraph External
    PayProv[Payment Provider]
    OtpProv[OTP or SMS Provider]
    MapsProv[Maps or Geocoding]
    SmtpProv[SMTP Provider]
  end

  Cust --> APIGW
  Resto --> APIGW
  AdminUI --> APIGW
  AuthClient -. bearer jwt .-> APIGW
  ...
  ProductSvc -- publish --> MQ
  IntegrationsSvc -- publish --> MQ

  EmailSvc -- consume --> MQ
  SocketGW -- consume --> MQ
  OrderSvc -- consume --> MQ
  PaymentSvc -- consume --> MQ
  ProductSvc -- consume --> MQ

  SocketGW -- websocket --> Cust
  SocketGW -- websocket --> Resto
  EmailSvc --> SmtpProv

  PaymentSvc --> IntegrationsSvc
  IntegrationsSvc --> PayProv
  PayProv --> IntegrationsSvc
  IntegrationsSvc --> OtpProv
  IntegrationsSvc --> MapsProv
```

### Email Service (component level)
```mermaid
flowchart TB
  subgraph EmailService
    QConsumer[Queue Consumer\nemail.send or order.updated]
    Templater[Template Renderer]
    SmtpSender[SMTP Transport]
    Dlq[Retry and DLQ]
    Metrics[Logs and Metrics]
    Health[/GET /healthz/]
  end

  MQ[(RabbitMQ)] --> QConsumer
  QConsumer --> Templater --> SmtpSender --> SmtpProv[(SMTP Provider)]
  QConsumer --> Dlq
  QConsumer --> Metrics
```

### Socket Gateway (component level)
```mermaid
flowchart TB
  subgraph SocketGateway
    AuthMW[JWT Verify Middleware]
    WsServer[Socket IO or WS Server]
    Rooms[Room Manager\nuser:id, branch:id]
    Health[/GET /healthz/]
  end

  MQ[(RabbitMQ)] --> Relay[Event Relay\norder.status.changed\npayment.updated\ninventory.updated]
  AuthMW --> WsServer --> Rooms
  Relay --> WsServer
```

### Integrations-3rd (component level)
```mermaid
flowchart TB
  subgraph IntegrationsService
    Api[Internal REST API]
    Adapter[Adapter Layer]
    Guard[Timeout Retry Circuit Breaker Idempotency]
    Health[/GET /healthz/]
  end

  Api --> Adapter --> Guard --> PayExt[Payment Provider]
  Api --> Adapter --> Guard --> OtpExt[OTP Provider]
  Api --> Adapter --> Guard --> MapsExt[Maps Provider]

  Webhook[/POST /webhooks/payment/] --> Api
  Api --> MQ[(RabbitMQ)]
```
